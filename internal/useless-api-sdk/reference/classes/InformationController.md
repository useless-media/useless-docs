...
> **new InformationController**(`configuration`): `InformationController`
...
> **basePath**: `string` = `'/api/v1/action/information'`
...
> **name**: `string` = `'InformationController'`
...
> **getPackageInformation**(`args`): `Promise`<`any`>
...
> **getWorkerInput**<`TQuery`>(`productInstanceId`, `action`, `query?`, `authorization?`, `correlationId?`): `Promise`<`any`>

#### Type Parameters

##### TQuery

`TQuery` *extends* `Partial`<[`WorkerInputQueryDto`](reference/interfaces/WorkerInputQueryDto.md)> & `Record`<`string`, `any`> = `Record`<`string`, `any`>
